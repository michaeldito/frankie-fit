...
begin
  ...
  select count(*)
  into total_tracked_users
  from public.profiles
  where role = 'user';

  return jsonb_build_object(
    'totalUsers', total_tracked_users,
    'realUsers', (
      select count(*)
      from public.profiles
      where role = 'user'
        and account_type = 'real_user'
    ),
    'internalTestUsers', (
      select count(*)
      from public.profiles
    ...
    'syntheticDemoUsers', (
      ...
      from public.profiles
      where role = 'user'
        and account_type = 'synthetic_demo'
    ),
    'onboardingCompleted', (
      select count(*)
      from public.profiles
    ...
    'onboardingCompletionRate', (
      case
        when total_tracked_users = 0 then 0
        else round(
          (
            (
              select count(*)
              ...
              where role = 'user'
                and onboarding_completed
            )::numeric / total_tracked_users::numeric
          ) * 100,
          1
        )
      end
...
begin
  ...
  select 'Onboarding incomplete'::text, count(*)::bigint, 'Tracked accounts that have not finished onboarding yet.'::text
  from tracked_users
  where not onboarding_completed

  union all

  select 'No diet logs in 7 days'::text, count(*)::bigint, 'Tracked accounts without a recent food update.'::text
  from tracked_users
  ...
  select 'No wellness check-in in 7 days'::text, count(*)::bigint, 'Tracked accounts without a recent energy or recovery check-in.'::text
  from tracked_users
  where onboarding_completed
  ...
  select 'Single-pillar usage only'::text, count(*)::bigint, 'Tracked accounts that only used one of the three pillars in the last 30 days.'::text
