...
export async function getDashboardData(context: CurrentAppContext): Promise<DashboardData> {
  ...
  const firstError =
    activityResult.error?.message ?? dietResult.error?.message ?? wellnessResult.error?.message ?? null;
  ...
  const computed = computeDashboardData(context.profile, activityLogs, dietLogs, wellnessCheckins);
