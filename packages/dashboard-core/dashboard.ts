...
export type DashboardProfileInput = Pick<
  ...
  "target_training_days" | "onboarding_completed" | "wellness_checkin_opt_in"
>;
...
export function createEmptyActivityDashboard(): ExerciseDashboardData {
  return {
    metrics: [
...
export function createEmptyDietDashboard(): DietDashboardData {
  return {
    metrics: [
...
function buildExerciseDashboardData(
  ...
  return {
    ...
    insight: buildExerciseInsight(profile, activityLogs, activityLogsThisWeek.length, activeDaysThisWeek),
    empty: false
  };
}
...
function buildWellnessDashboardData(wellnessCheckins: WellnessCheckinRow[]): WellnessDashboardData {
  ...
  const weekStartKey = toDateKey(getWeekStart());
  const checkinsThisWeek = wellnessCheckins.filter((checkin) => checkin.logged_for_date >= weekStartKey);
...
/** Pure computation over already-fetched rows. Callers own fetching (RLS, error handling) and pass the results in. */
...
/** Inclusive lower bound (as a date key) for "recent" queries, e.g. the last 30 days. */
export function getSinceDateKey(daysBack: number): string {
  return toDateKey(addDays(getPacificToday(), -(daysBack - 1)));
}
